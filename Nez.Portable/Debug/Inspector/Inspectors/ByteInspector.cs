...
namespace Nez
{
	public class ByteInspector : Inspector
	{
		...
		void SetupSlider(Table table, Skin skin, float leftCellWidth, float minValue, float maxValue, float stepSize)
		{
			...
			_slider.SetValue(GetValue<byte>());
			_slider.OnChanged += newValue => { _setter.Invoke((byte) newValue); };
		...
		{
			if (_textField != null)
				_textField.SetText(GetValue<byte>().ToString());
			if (_slider != null)
				_slider.SetValue(GetValue<byte>());
		}
	}
